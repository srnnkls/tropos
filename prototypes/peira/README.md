...
> *πεῖρα* • feminine noun, first declension; from Proto-Indo-European \*per- ("to try, to risk")
>
> Pronunciation: /pêː.ra/ (Classical), /ˈpi.ra/ (Modern)
>
> a trial, an attempt, a test; the experience gained by trying

The root gives us *empirical*, *experience* (via Latin *experīrī*), *peril*, and *pirate* (πειρατής,
one who makes an attempt).
...
versioned rule artifact comes out; `peira check` runs it against files, diffs or documents and emits
...
peira collapses that matrix in three ways, cheapest first:
...
guides / specs / ADRs / policies           source / diff / prose
 ...
 .peira/rules.json + manifest.json                        │
...
uv run peira compile examples/guides --prefix EX -o .peira
uv run peira compile ~/.cache/tropos/loqui/languages/python --prefix LOQ-PY -o .peira-loqui

# check sources against it
uv run peira check examples/sources
uv run peira check src/ --diff HEAD~1
uv run peira check README.md --format sarif > results.sarif

# inspect
uv run peira rules            # every compiled rule, its evaluator and where it came from
uv run peira features doc.md  # dump the semantic IR per chunk
```

Set `TYPESAFE_API_KEY` to use Jev for compilation, feature extraction and judge rules. Without a
key, or with `--offline`, peira runs the native detectors plus a keyword compiler and a heuristic
...
`peira compile` writes two files:
...
`src/peira/features.py` is the vocabulary. Each feature is a Jev question with a fixed answer type
and a polarity:
...
subsumption and emits a gate decision. peira shares its engineering philosophy but not its
...
- Different primitive: fas asks "does this CUE pattern subsume this value?", a closed-world,
  deterministic check over one event. peira asks "does this region satisfy this normative
  sentence?", a probabilistic judgment over a corpus, with confidence thresholds and provenance
  back to prose. fas's output algebra is a gate (allow, deny, ask, inject, modify); peira's is a
  diagnostic (severity, evidence, confidence, source).
- Narrow bridges worth considering later, none of them architecture: (1) peira's compiler could
  ...
  without changing fas's decision model; (3) peira should adopt fas's compile-time strictness and
