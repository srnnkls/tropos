...
The reply → resolve workflow, and the `fcp` (fix → commit → push) chain it runs first, live in their own skills. Invoke Skill `fcprr`, forwarding the `$ARGUMENTS` after the leading `fcprr` token (`--comment <id>…`, `--reply`, `-m`, `--pr`, `paths…`).
